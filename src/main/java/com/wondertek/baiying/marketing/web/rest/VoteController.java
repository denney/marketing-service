...
public class VoteController {
	...
	public static Map<String, Object> resultJson(int resCode, String resMessage,
			Object objectValue) {
		Map map = new HashMap();
	...
	public Map<String, Object> findAll(int pageNum, int pageSize,
		...
		Page<Vote> values = voteService.findAll(pageNum, pageSize, direction,
				property);
	...
	@ResponseBody
	public Map saveImgs(MultipartFile posterImg_file,
			HttpServletRequest request, Vote vote) {
		...
		if (vote.getType().equals("1")) {
			...
			// 3 判断选项个数与图片个数是否一致
			// 3.1 获取图片个数
			List<MultipartFile> optionImg_files = ((MultipartHttpServletRequest) request)
					.getFiles("file");
			System.out.println("item.length:" + t.length);

			System.out
					.println("optionImg_files.size:" + optionImg_files.size());
...
//            获取海报图片
            // 获取文件名,求得后缀名，图片重命名
             fileName = getFileName(posterImg_file);
...
//        1.先判断选项有图片还是没图片,type为1说明选项有图片
        if (vote.getType().equals("1")) {
//              图片如何更新？
//            截取字符串，以选项作为name变量判断文件是否为空，如果不为空就更新，为空就不更新
...
//                    字符串截取异常
                    return resultJson(108,"failure",vote);
                }
